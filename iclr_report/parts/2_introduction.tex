...
\textbf{Exponential Moving Average (EMA)}: Another method for averaging weights over training is EMA~\cite{ema}. Here, the weights at each training step are combined with the previous average using an exponential decay.
...
\textbf{How much shared training is required for souping to be effective?} Models trained from the same pre-trained checkpoint can soup effectively, but how much shared training is required?\\
%  We investigate varying the number of shared pre-training epochs before splitting into fine-tuned variants.
...
\textbf{Is souping transitive?} If model $A$ soups with $B$, and $B$ soups with $C$, will $A$ soup well with model $C$? \\
...
Further experiments investigating the effect of permuting models prior to souping and how souping affects robustness can be found in Appendix~\ref{appendix:permutation_alignment} and~\ref{fig:soup_robustness_cdf} respectively.
