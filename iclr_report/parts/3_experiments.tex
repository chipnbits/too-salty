...
\textbf{1. Soup Gain and Shared Epochs:} Figure~\ref{fig:soup_gain_cdf} shows the CDF of soup gains.Many soups have extreme behaviour, with $40\%$ of soups losing over 60\% and only $14\%$ with positive gain. There is also a sharp transition to collapse, with only $20\%$ soups between $-70\%$ and $-20\%$ gain. Grouping by the shared number of training epochs before branching (e.g., ingredients branched at epochs 50 and 100 share 50 epochs), we find the probability of positive soup gain generally increases with shared epochs, reaching around $80\%$ after 280 shared epochs, see Figure~\ref{fig:soup_gain_probabilities}. At this stage, no soups drop accuracy by more than $5\%$. However, peak gains $(>0.5\%)$ require a balance; if shared training is too extensive ingredients are not diverse enough, performance diminishes. Further analysis on mean gain, loss metrics, and model collapse is provided in Figures~\ref{fig:soup_gain_conditional_exp}, ~\ref{fig:soup_gain_quantiles_loss} and~\ref{fig:model_collapse}.
...
\textbf{2. Predicting Soupability with Similarity:} To test if soupability is predictable, we compute a variety of similarity and distance metrics between model pairs. All metrics perform similarly, see Figure~\ref{fig:soup_gain_vs_all_metrics}. Using KL divergence between the outputs of the ingredients as an example, we find a strong negative correlation with soup gain (Spearman $-0.88$), indicating divergence often leads to model collapse, see Figure~\ref{fig:soup_gain_vs_kl_logits}. By contrast, gain among only positive soups has a positive correlation with dissimilarity (Spearman $0.16$) and the most positive soups have some diversity, see Figure~\ref{fig:soup_gain_vs_kl_positive_soups}. Effective souping requires models to be sufficiently similar, but also rewards variety. Balancing these two effects is key to tasty soups. Additionally, shared epochs correlate with all similarity metrics, see Figure~\ref{fig:kl_shared_epochs}.
...
\textbf{3. Is Souping Transitive?:} To test the hypothesis that soupable models lie in the same low-loss basin, we evaluate the transitivity of model triplets $(A, B, C)$. Figure~\ref{fig:transitivity} shows that $B$ and $C$ are likely to soup only if $A$ soups with both $B$ and $C$. We observe a moderate positive correlation (Spearman $0.64$) between the gain of $(B,C)$ and the minimum gain of $(A, B)$ and $(A, C)$, see Figure~\ref{fig:transitivity_min}. 
...
We embed our $104$ models into 2D using soup gain as a distance metric to search for souping clusters, plotting them in Figure~\ref{fig:model_embedding}, with details found in Section~\ref{sec:model_embedding_creation}. We find that all successful soups form a single connected component, and there is a dense cluster of models with higher shared epochs. 
% There are many counter-examples, but this supports the conclusion that souping is moderately transitive and successful ingredients generally lie within a single loss basin.
Cases where transitivity fails exhibit a smooth but significant loss barrier, showing that the loss basin is non-convex with modes of interconnectivity, see Figure \ref{fig:transitivity_failure_raster}. We also find that such failures never exhibits model collapse, see Figure~\ref{fig:transitivity_neg_edge_acc_cdf}. We attempt to explain when transitivity fails in terms of the shared epochs and find further evidence of the sufficient-diversity tradeoff, see Tables~\ref{tab:shared_epoch_triples_transitivity} and~\ref{tab:epoch_diff_triples_transitivity}. 
...
Here we compare the improvements to SWA in Table~\ref{tab:swa_compare}. SWA offers a significant boost in robustness to corruption, always improves the baseline run, and performs better than the best soups. We hypothesize that this is because in this experimental setting, SWA has explored a wider breadth than the binary soups and has more closely converged to the low-loss center described by \citet{modelstock}.
...
\midrule
\textbf{Soups: 200 Epochs} & $39.7\% \pm 8.4\%$ & $0.37\% \pm 0.06\%$ & $0.42\% \pm 0.12\%$ \\
\addlinespace
\textbf{Soups: 250 Epochs} & $44.7\% \pm 11.2\%$ & $0.49\% \pm 0.07\%$ & $0.53\% \pm 0.09\%$ \\
\addlinespace
\textbf{SWA} (12 Runs) & $100\%$ & $1.7\% \pm 0.3\%$ & $4.3\% \pm 0.5\%$ \\
\bottomrule
\end{tabular}
\caption{Comparison of souping robustness to SWA. Note that gain is the mean over only positive soup. Including all soups would result in negative performance, as there are many instances of model collapse. SWA performs notably better. We hypothesise SWA explores a wider region of the low-loss basin than a binary soup can.}
