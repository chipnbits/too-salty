...
Our experiments are designed to recreate the branching effect of several training trajectories, but with a wider variety of models and shared training steps than a single pre-trained model would provide.
...
We find that all metrics perform similarly. A plot for all metrics can be found in Figure~\ref{fig:soup_gain_vs_all_metrics}. We arbitrarily choose to show the KL divergence between the logits in Figure~\ref{fig:soup_gain_vs_kl_logits}. We observe a strong correlation between KL divergence and soup gain with a Spearman correlation between of -0.86. However, many of these soups have terrible performance. Figure~\ref{fig:soup_gain_vs_kl_positive_soups} shows the soup gain against KL divergence for only those models with positive soup gain. Here, we observe a moderate positive correlation with a Spearman correlation of 0.39. We conclude that models must be sufficiently similar in order to soup, but in order for souping to be effective, the models must also be sufficiently different. Balancing these two effects is key to effective souping.
...
If model $A$ soups successfully with models $B$ and $C$, will $B$ and $C$ also soup successfully? If it is true that models soups together when they lie in the same low loss region, then transitivity holds. To test this, we consider all triplets of models $(A, B, C)$. We plot the probability that $B$ and $C$ soup against the number of positive soups involving $A$ in Figure~\ref{fig:transitivity}. We observe a strong positive correlation, with the probability that $B$ and $C$ soup increasing as the number of positive soups involving $A$ increases. This suggests that souping is approximately transitive.
...
We also plot the soup gain between $B$ and $C$ against the minimum soup gain of $(A, B)$ and $(A, C)$ in Figure~\ref{fig:transitivity_min}. We observe a moderate positive correlation, with Spearman correlations of 0.64. This further supports the hypothesis that souping is approximately transitive.

Given this evidence for transitivity, we consider the broader landscape of all $104$ of our originally trained models. Do the soups all exist in separate clusters of models in separate loss basins? We define a distance metric defined as $d_{AB} = -\operatorname{sign}(\text{soup gain}) - 0.1 * \text{soup gain}$. Intuitively, this metric puts models close together that soup together positively, taking into account the magnitude of soup gain. We then cast this down into a 2-dimensional embedding using Multidimensional Scaling. We also color by branching epoch. The model embedding is plotted in Figure~\ref{fig:model_embedding}. We find that all successful soups lie in a single cluster of models all branched from later on in the training procedure. 
...
All experiments thus far have measured soup gain on a held-out test set. However, souping has also been used for robustness to distribution shift~\citep{seasoningsoups}. We now wish to establish whether souping for in-distribution performance also increases out-of-distribution performance. We compute the soup gain on CIFAR-100C~\citep{cifar100c} with severity level 3. We find that the soup gains on test and corrupted data correlate very well, with a Spearman correlation of 0.99. A scatterplot can be found in Figure~\ref{fig:test_vs_corrupted}. The positive trend still holds when conditioning on only soups with positive soup gain on the test set. We find a Pearson correlation of $0.61$. A plot can be found in Figure~\ref{fig:test_vs_corrupted_pos}. We conclude that model gains across the test set and corrupted data are strongly correlated.
...
We also plot the probability of positive soup gain on corrupted data as a function of shared epochs in Figure~\ref{fig:soup_corrupted_positive_gain_prob}. We see that the probability of positive gain increases with the number of shared epochs, for both clean and corrupted data. However, the corrupted data consistently has a slightly lower probability of positive gain.

We also wanted to test whether souping can help reduce the \emph{robustness gap}, the gap between the test set and corrupted set loss. We find mean and median difference in robustness gap before and after souping of approximately 0. A plot of the CDF can be found in Figure~\ref{fig:soup_robustness_cdf}. We conclude that souping does not reduce the robustness gap.
